<script lang="ts">
	...
	export let defaultSelected: string[]; // list of author names
	...
	const {
		...
		forceVisible: true,
		multiple: true,
		defaultSelected: defaultSelected.map((author) => {
			const elem = mangas.find((manga) => manga.author === author);
			if (!elem) throw new Error(`Author ${author} not found in mangas list`);
			return { value: elem, label: elem.title };
		})
	});
...
<div class="flex flex-col gap-1">
	...
	<div class="relative">
		{#if $selected}
			{#each $selected as { label }, idx}
				{label}
				<button
					type="button"
					on:click={() => {
						if (!Array.isArray($selected)) return;
						$selected = $selected.filter((e, i) => i !== idx);
					}}
					class="ml-0.5 inline-flex h-4 w-4 flex-shrink-0 items-center justify-center rounded-md text-spur-blue-400 hover:bg-spur-blue-200 hover:text-spur-blue-500 focus:bg-spur-blue-500 focus:text-white focus:outline-none"
				>
					<span class="sr-only">Remove large option</span>
					<svg class="h-2 w-2" stroke="currentColor" fill="none" viewBox="0 0 8 8">
						<path stroke-linecap="round" stroke-width="2" d="M1 1l6 6m0-6L1 7" />
					</svg>
				</button>
			{/each}
		{/if}
